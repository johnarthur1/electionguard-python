...
package-upload-test-ci:
	python -m pip install --user --upgrade twine
	python -m twine upload --repository testpypi --username __token__ --password ${{ secrets.TEST_PYPI_TOKEN }} dist/*
...
release-version-ci:
	...
	echo ::set-env name=PACKAGE_VERSION::$(echo $VERSION | python setup.py --version)
